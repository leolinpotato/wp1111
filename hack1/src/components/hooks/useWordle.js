...
const useWordle = (solution) => {
    ...
    const handleEnter = () => {
        let solution_letter_set = solution.split('');
        ...
        // green
        for (let i = 0; i < 5; i++) {
            if (newGuess[i].char === solution_letter_set[i]) {
                newGuess[i].color = "green";
                solution_letter_set[i] = "green";
            }
        }
        // yellow
        for (let i = 0; i < 5; i++) {
            if (newGuess[i].color === "green")
                continue;
            for (let j = 0; j < 5; j++) {
                if (newGuess[i].char === solution_letter_set[j]) {
                    newGuess[i].color = "yellow";
                    solution_letter_set[j] = "yellow";
                    break;
                }
            }
        }

        guesses[turn] = newGuess;
        ...
        let newUseChar = usedChars;
        for (let i = 0; i < 5; i++) {
            if (solution_letter_set[i] === "yellow" && !usedChars[solution[i]])
                newUseChar[solution[i]] = "yellow";
        }
        for (let i = 0; i < 5; i++) {
            if (solution_letter_set[i] === "green" && (!usedChars[solution[i]] || usedChars[solution[i]] === "yellow"))
                newUseChar[solution[i]] = "green";
        }
        setUsedChars(newUseChar);
    }
